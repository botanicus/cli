...
namespace :version do
  task :increase do
    puts "Which version are you just releasing (previous version is #{CLI::VERSION})"
    version = STDIN.gets.chomp
    File.open("lib/cli/version.rb", "w") do |file|
      file.puts <<-EOF
# encoding: utf-8
...
module CLI
  VERSION ||= "#{version}"
    ...
    CLI.const_set("VERSION", version) # so other release tasks will work
    sh "git commit lib/cli/version.rb -m 'Increased version to #{version}'"
  end
end
